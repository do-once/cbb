...
class DoonceNum2chn {
   ...
   */
  getDecimalPartChn(n) {
    if (!(0, utils_1.isSafeNumber)(Number(n)) || typeof n !== 'string') {
      return '参数为无法解析'
    }
    return String(n)
      .split('')
  ...
  getIntergerSections(n) {
    let num = Number(n)
    if (!(0, utils_1.isSafeNumber)(num) || num < 0) throw new Error('参数无法解析或小于0')
    if (num === 0) return [0]
    let temp = []
    while (num > 0) {
      temp.push(num % 10000)
  ...
  getSectionsChn(section) {
    ...
    if (!(0, utils_1.isSafeNumber)(num) || num >= 10000 || num < 0) {
    ...
    while (num > 0) {
      ...
      if (g === 0) {
        if (needPadStartZero) {
          ...
          str = (0, utils_1.padStr)(str, 1, this.getNumChar(0))
        }
      } else {
  ...
  getIntergetPartChn(intergerPart) {
    ...
    if (!(0, utils_1.isSafeNumber)(originNum) || originNum < 0) {
      ...
      .reduce((acc, cur, index, arr) => {
        ...
        let sectionChn =
          index !== arr.length - 1 && cur < 1000
            ? (0, utils_1.padStr)(this.getSectionsChn(cur), 1, this.getNumChar(0))
  ...
  transform(n) {
    const num = Number(n)
    if (!(0, utils_1.isSafeNumber)(num)) return '无法解析成数字或超出范围'
    const { sign, interger, dot, decimal } = (0, utils_1.getNumParts)(num)
...
exports.default = DoonceNum2chn
//# sourceMappingURL=Num2Chn.js.map
