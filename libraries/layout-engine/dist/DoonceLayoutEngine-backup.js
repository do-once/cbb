...
import { Char, Formula, FormulaRenderTypeEnum, Graph, ImgPlaceholder, ImgSurrounTypeEnum, LayoutItemTypeEnum, Row } from './layout-item';
export class DoonceLayoutEngine {
    ...
    constructor({ globalFontOptions, inputLayoutItemDescList, formulaRenderType, debug }) {
        if (!globalFontOptions)
            throw new Error('globalFontOptions is required');
    ...
    async init() {
        ...
        if (!this.isFontLoaded())
            throw new Error(`font ${this.globalFontOptions.fontFamily} load faild`);
        /** 实例化用户传入的布局项 */
